                    ...
                    <div className="overflow-y-auto p-2 flex flex-col gap-2 flex-1">
                      {(() => {
                        ...
                        if (cat === 'Concepts') {
                          ...
                          // continent when it is a known section, otherwise the option
                          // name itself for legacy section-named values, else 'Other'.
                          const sectionOf = (opt: string) => {
                            const owner = availableFlags.find((f) => f.country === opt);
                            if (owner?.continent && sections.includes(owner.continent)) {
                              return owner.continent;
                            }
                            if (sections.includes(opt)) return opt;
                            return 'Other';
                          };

                          const matchesSearch = (opt: string) =>
                            opt.toLowerCase().includes(submenuSearch.toLowerCase());
                          const searching = submenuSearch.trim() !== '';

                          const groups = [...sections, 'Other'].map((section) => ({
                            section,
                            options: allOptions
                              .filter((opt: any) => sectionOf(opt) === section && matchesSearch(opt))
                              .sort(),
                          })).filter((g) => g.section !== 'Other' ? !searching || g.options.length > 0 : g.options.length > 0);

                          const totalMatches = groups.reduce((n, g) => n + g.options.length, 0);
                              ...
                              {groups.map((group) => (
                                <div key={group.section} className="flex flex-col gap-1.5">
                                  ...
                                  {group.options.length > 0 ? (
                                    ...
                                    <div className="text-[11px] italic text-zinc-400 dark:text-zinc-500 px-0.5">
                                      No sub-categories yet — add one from the flag editor.
                                    </div>
                                  )}
                                </div>
                              ))}
